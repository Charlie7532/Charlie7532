...
export const HeaderNav: React.FC<HeaderNavProps> = ({ data }) => {
  ...
  const menuPosition = (data?.settings as any)?.menuPosition || 'right'
  const showSearchBar = (data?.settings as any)?.showSearchBar !== false // Default to true
  const buttons = (data?.settings as any)?.buttons || []

  // Get text color styling to ensure consistency with header
  const getTextColorStyles = () => {
    const textColor = data?.settings?.textColor || 'auto'
    let textClasses = ''
    let linkClasses = ''
    ...
    switch (textColor) {
      case 'primary':
        ...
        break
      case 'custom':
        const customColor = data?.settings?.customTextColor
        if (customColor) {
